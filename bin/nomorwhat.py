...
def NUM_rules(window, curr_POS):
    ...
    param:
    window: list of two tuples, containing the first and second token/lemma/annotation after the given word.
    curr_POS: the annnotation of the currently analysed token
...
def NOUN_rules(window, curr_POS, token):
    ...
    if check_macro('NUs', window[0]):  # if the next token is postposition(al like element), the word gets a 'none'
        ...
        if check_macro('PSE', window[1]) and not re.search('PSe3', curr_POS):
            curr_POS = curr_POS.replace('suff', 'gen')            
        elif check_macro('V', window[1]) or check_macro('PUNCT', window[1]) or check_macro('relpron',
        ...
        curr_POS = curr_POS.replace('NOM', 'suff')
        if check_macro('V', window[1]) or check_macro('PUNCT', window[1]) or check_macro('relpron', window[1]) or \
...
def nom_or_what(s, macro):
    ...
    return:
    new_sent: a list of the tokens of the sentence with the novel tags (word/lemma/annotation)
    to_write_later: a list that stores tuples which contain the window of each suffixless nominal
    and their new annotation; this is to help creating the annotation file
    """   
    global macros
    macros = macro
    sent = format_sents(s)
